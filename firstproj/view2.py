...
def get_credential(request):
    ...
    vk_url = f"https://oauth.vk.com/authorize?client_id={settings.VK_CLIENT_ID}&display=page&scope=offline&redirect_uri={redirect_url}"
...
def get_token(request):
    if request.method == 'GET':
        ...
        first_name = user_info['response'][0]['firs' \
                                              't_name']
